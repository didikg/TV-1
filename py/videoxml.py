...
def scrape_tv_programming(channel_id, date, session=None, max_attempts=5):
    if session is None:
        ...
        try:
            ...
            for item in programming_items:
                try:
                    ...
                    if not start_time:
                        continue
                    duration = parse_duration(item) or 30
            ...
            print(f"Error fetching data for {channel_id} on {date} (attempt {attempt+1}/{max_attempts}): {str(e)}", file=sys.stderr)
        ...
        if programs:
            ...
            for program in programs:
                try:
                    ...
                    duration = program.get('duration', 30)
